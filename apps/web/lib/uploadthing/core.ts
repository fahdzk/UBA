...
const publicMiddleware = async () => {
  return { userId: "anonymous" as string };
};
...
export const ourFileRouter = {
  ...
  // Headshot — required, JPG/PNG, max 8MB
  headshotUploader: f({ image: { maxFileSize: "8MB", maxFileCount: 1 } })
    .middleware(publicMiddleware)
    .onUploadComplete(async ({ file }) => {
      return { url: file.url, key: file.key };
    }),

  // Full body shot — required, JPG/PNG, max 8MB
  fullBodyUploader: f({ image: { maxFileSize: "8MB", maxFileCount: 1 } })
    .middleware(publicMiddleware)
    .onUploadComplete(async ({ file }) => {
      return { url: file.url, key: file.key };
    }),

  // Resume — optional, PDF/DOC/DOCX, max 8MB
  resumeUploader: f({
  ...
  // Intro video — optional, MP4/MOV, max 64MB
